"""
Standalone cloud tuning script for the notebook-faithful MACD-BB-ATR strategy v1.2.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--deploy-start", default="2025-01-01")
    parser.add_argument("--deploy-end", default="2026-03-30")
    ...
    parser.add_argument("--use-default-params", action="store_true", help="Skip Optuna and run the app directly with DEFAULT_PARAMS.")
...
def data_download_start(train_start: str, deploy_start: str) -> str:
    train_prev_year = pd.Timestamp(f"{pd.Timestamp(train_start).year - 1}-01-01")
    earliest_needed = min(train_prev_year, pd.Timestamp(deploy_start))
...
def valid_params(params: dict) -> bool:
    ...
    if params["macd_k_mid"] >= params["macd_k"]:
        return False
    if params["trail_tp"] and not params["use_trailing"]:
        return False
...
def slice_portfolio_df(portfolio_df: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    if portfolio_df is None or portfolio_df.empty:
        return pd.DataFrame()
    start_ts = pd.Timestamp(start)
    end_ts = pd.Timestamp(end)
    return portfolio_df.loc[(portfolio_df.index >= start_ts) & (portfolio_df.index <= end_ts)].copy()


def yearly_sharpe_map(portfolio_df: pd.DataFrame, start_year: int, end_year: int) -> dict[int, float | None]:
    out: dict[int, float | None] = {}
    for year in range(start_year, end_year + 1):
        year_start = f"{year}-01-01"
        year_end = f"{year}-12-31"
        year_df = slice_portfolio_df(portfolio_df, year_start, year_end)
        sharpe = compute_sharpe(year_df, risk_free=RISK_FREE)
        out[year] = round(float(sharpe), 4) if np.isfinite(sharpe) else None
    return out


def run_train_select_deploy_backtest(
    data_cache: dict[str, pd.DataFrame],
    params: dict,
    train_start: str,
    train_end: str,
    ...
    train_df, train_strategy, _ = run_multi_stock_backtest_unified(
        data_cache=data_cache,
        tickers=TICKERS,
        start=train_start,
        end=train_end,
    ...
    selection = optimize_portfolio(
        stock_equity_history=train_strategy.stock_equity_history,
        risk_free=RISK_FREE,
        top_k=top_k,
    )
    selected_tickers = selection.get("selected_tickers") or list(TICKERS)
    allocations = selection.get("allocations") or {t: round(1 / len(selected_tickers), 4) for t in selected_tickers}

    deploy_df, deploy_strategy, _ = run_multi_stock_backtest_unified(
        data_cache=data_cache,
    ...
    return train_df, deploy_df, {
        "selected_tickers": selected_tickers,
        "allocations": allocations,
        "raw_weights": selection.get("raw_weights", []),
...
def evaluate_candidate(
    ...
    train_df, deploy_df, selection = run_train_select_deploy_backtest(
        data_cache=data_cache,
        params=params,
    ...
    if train_df is None or train_df.empty or deploy_df is None or deploy_df.empty:
        return {}

    train_start_year = pd.Timestamp(train_start).year
    train_end_year = pd.Timestamp(train_end).year
    deploy_start_year = pd.Timestamp(deploy_start).year
    deploy_end_year = pd.Timestamp(deploy_end).year

    train_year_sharpes = yearly_sharpe_map(train_df, train_start_year, train_end_year)
    deploy_year_sharpes = yearly_sharpe_map(deploy_df, deploy_start_year, deploy_end_year)

    valid_train = [value for value in train_year_sharpes.values() if value is not None]
    valid_deploy = [value for value in deploy_year_sharpes.values() if value is not None]
    train_avg_sharpe = float(np.mean(valid_train)) if valid_train else float("-inf")
    deploy_avg_sharpe = float(np.mean(valid_deploy)) if valid_deploy else float("-inf")
    deploy_total_sharpe = compute_sharpe(deploy_df, risk_free=RISK_FREE)

    return {
        "train_df": train_df,
        "deploy_df": deploy_df,
        "selection": selection,
        "train_year_sharpes": train_year_sharpes,
        "deploy_year_sharpes": deploy_year_sharpes,
        "train_avg_sharpe": train_avg_sharpe,
        "deploy_avg_sharpe": deploy_avg_sharpe,
        "deploy_total_sharpe": float(deploy_total_sharpe) if np.isfinite(deploy_total_sharpe) else float("-inf"),
        "train_summary": compute_summary(train_df, initial_capital, risk_free=RISK_FREE),
        "deploy_summary": compute_summary(deploy_df, initial_capital, risk_free=RISK_FREE),
    }
...
def objective(
    ...
    train_avg = result["train_avg_sharpe"]
    deploy_sharpe = result["deploy_total_sharpe"]
    deploy_avg = result["deploy_avg_sharpe"]

    if not np.isfinite(train_avg) or not np.isfinite(deploy_avg) or not np.isfinite(deploy_sharpe):
        return -1e9
    if not (train_avg > deploy_sharpe and deploy_sharpe > 1.2):
        return -1e9

    score = float(train_avg + deploy_avg)
    trial.set_user_attr("train_avg_sharpe", round(train_avg, 4))
    trial.set_user_attr("deploy_avg_sharpe", round(deploy_avg, 4))
    trial.set_user_attr("deploy_total_sharpe", round(result["deploy_total_sharpe"], 4))
    trial.set_user_attr("train_year_sharpes", result["train_year_sharpes"])
    trial.set_user_attr("deploy_year_sharpes", result["deploy_year_sharpes"])
    trial.set_user_attr("selected_tickers", result["selection"]["selected_tickers"])
    trial.set_user_attr("allocations", result["selection"]["allocations"])
...
def save_outputs(output_dir: Path, config: dict, best_params: dict, study: optuna.Study,
                 oos_df: pd.DataFrame, period_rows: list[dict], overall_summary: dict) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    (output_dir / "period_summary.json").write_text(json.dumps(period_rows, indent=2))
    ...
    if not oos_df.empty:
        ...
        plot_oos_equity(export_df, output_dir / "oos_equity_2025_now.png", chart_start=CHART_START_DATE)


def plot_oos_equity(oos_df: pd.DataFrame, output_path: Path, chart_start: str = CHART_START_DATE) -> None:
    chart_df = oos_df.copy()
    chart_df.index = pd.to_datetime(chart_df.index)
    chart_df = chart_df.loc[chart_df.index >= pd.Timestamp(chart_start)]
    if chart_df.empty or "PortfolioValue" not in chart_df.columns:
        logger.warning("Skipping OOS equity chart export because no data is available from %s", chart_start)
        return

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(chart_df.index, chart_df["PortfolioValue"], color="#0f766e", linewidth=2.0)
    ax.set_title(f"Out-of-Sample Equity Curve ({chart_start} to now)")
    ax.set_xlabel("Date")
...
def main() -> None:
    ...
    deploy_end_ts = pd.Timestamp(args.deploy_end)
    deploy_start_ts = pd.Timestamp(args.deploy_start)
    train_start_ts = pd.Timestamp(args.train_start)
    train_end_ts = pd.Timestamp(args.train_end)
    if train_end_ts <= train_start_ts:
        raise ValueError("train-end must be after train-start")
    if deploy_end_ts <= deploy_start_ts:
        raise ValueError("deploy-end must be after deploy-start")
    if deploy_start_ts <= train_end_ts:
        raise ValueError("deploy-start must be after train-end for notebook-style train/test separation")

    config = {
        "train_start": args.train_start,
        "train_end": args.train_end,
        "deploy_start": args.deploy_start,
        "deploy_end": args.deploy_end,
        "top_k": args.top_k,
        "initial_capital": args.initial_capital,
        "strategy_version": STRATEGY_VERSION,
        "use_default_params": args.use_default_params,
        "constraint": {
            "train_avg_sharpe_gt_oos_sharpe_2025_2026": True,
            "oos_sharpe_2025_2026_gt_1_2": True,
            "objective": "maximize(train_avg_sharpe + deploy_avg_sharpe)",
        },
    }
    logger.info("Run config: %s", config)

    data_start = data_download_start(args.train_start, args.deploy_start)
    data_end = (deploy_end_ts + pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    data_cache = download_data(TICKERS, data_start, data_end)

    if args.use_default_params:
        logger.info("Skipping Optuna and using DEFAULT_PARAMS directly.")
        ...
        tuned_params = dict(study.best_trial.params)
        if tuned_params["macd_signal"] > tuned_params["macd_fast"]:
            tuned_params["macd_signal"] = tuned_params["macd_fast"]

        if not np.isfinite(study.best_value) or study.best_value <= -1e8:
            logger.warning("No valid Optuna trial found. Using DEFAULT_PARAMS.")
            best_params = dict(DEFAULT_PARAMS)
        else:
    ...
    if not final_result:
        raise RuntimeError("Final evaluation failed for the selected parameter set.")

    oos_df = final_result["deploy_df"]
    train_year_sharpes = final_result["train_year_sharpes"]
    deploy_year_sharpes = final_result["deploy_year_sharpes"]
    period_rows = [
        {
            "period": f"train_{train_start_ts.year}_{train_end_ts.year}",
            "start": args.train_start,
            "end": args.train_end,
            "avg_sharpe": round(final_result["train_avg_sharpe"], 4),
            "year_sharpes": train_year_sharpes,
        },
        {
            "period": f"deploy_{deploy_start_ts.year}_{deploy_end_ts.year}",
            "start": args.deploy_start,
            "end": args.deploy_end,
            "avg_sharpe": round(final_result["deploy_avg_sharpe"], 4),
            "year_sharpes": deploy_year_sharpes,
            "selected_tickers": final_result["selection"]["selected_tickers"],
            "allocations": final_result["selection"]["allocations"],
        },
    ]
    overall_summary = final_result["deploy_summary"]
    overall_summary["study_best_value"] = round(float(study.best_value), 4) if np.isfinite(study.best_value) else None
    overall_summary["train_avg_sharpe"] = round(final_result["train_avg_sharpe"], 4)
    overall_summary["deploy_avg_sharpe"] = round(final_result["deploy_avg_sharpe"], 4)
    overall_summary["deploy_total_sharpe"] = round(final_result["deploy_total_sharpe"], 4)
    overall_summary["train_year_sharpes"] = train_year_sharpes
    overall_summary["deploy_year_sharpes"] = deploy_year_sharpes
    overall_summary["selected_tickers"] = final_result["selection"]["selected_tickers"]
    overall_summary["allocations"] = final_result["selection"]["allocations"]

    save_outputs(output_dir, config, best_params, study, oos_df, period_rows, overall_summary)

    print("\nBest params")
    print(json.dumps(best_params, indent=2))
    print("\nTrain year Sharpe")
    print(json.dumps(train_year_sharpes, indent=2))
    print("\nDeploy year Sharpe")
    print(json.dumps(deploy_year_sharpes, indent=2))
    print("\nOverall summary")
